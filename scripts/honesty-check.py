...
RED_FLAGS = [
    ...
    # 瞎编命令输出（没有实际运行就说输出内容）
    (r"`[^`]*`.*的输出是", "描述命令输出但未实际运行"),
    (r"(ls|git|cat|ps|df)\s+[^|>\n]+.*输出.*", "描述命令输出但未实际运行"),
    (r"会显示.*如下", "描述命令/文件输出但未实际运行"),

    # 瞎编文件内容（没有读取就说内容）
    (r"文件内容是.*如下", "描述文件内容但未实际读取"),
    (r"配置文件.*包含.*", "描述配置文件内容但未实际读取"),
    (r"代码.*包含.*", "描述代码内容但未实际读取"),

    # 瞎编具体数值
    (r"\d+[\u4e00-\u9fa5]?\s*(ms|秒|分钟|小时|天)", "使用了时间数值但未实测"),
    (r"大约?\s*\d+(\.\d+)?\s*(MB|GB|KB|GB)", "使用了内存/文件大小数值但未实测"),
    (r"占\d+%", "使用了百分比但未实测"),
    (r"需要\s*\d+\s*(步|次|轮)", "使用了步骤/次数但未实测"),

    # 瞎编错误信息
    (r"错误信息是.*", "描述错误信息但未实际看到"),
    (r"报错.*(Error|Exception|panic).*", "描述报错但未实际运行"),

    # 瞎编路径/端口/配置值
    (r"端口[是为]?\s*\d{4,5}", "描述端口但未确认"),
    (r"IP[是为]?\s*\d+(\.\d+){3}", "描述IP地址但未确认"),
]
...
YELLOW_FLAGS = [
    ...
    # 推测性表述
    (r"应该\S*\s*", "使用了推测性表述「应该」"),
    (r"大概\S*\s*", "使用了推测性表述「大概」"),
    (r"可能\S*\s*", "使用了推测性表述「可能」"),
    (r"一般来说", "以一般性推断代替具体事实"),
    (r"通常.*是", "以通常情况代替实际数据"),
    (r"多数情况下", "以多数情况代替实际数据"),

    # 敏感操作建议
    (r"建议.*(rm|kill|drop|truncate|alter)", "建议执行危险操作"),
    (r"可以.*(重启|删除|修改|禁用)", "建议执行可能破坏性操作"),
    (r"直接\s*", "建议绕过安全步骤"),
    (r"手动.*(删除|修改|清空)", "建议手动执行危险操作"),

    # 未经确认的配置/参数
    (r"参数.*是\s*\S+", "描述参数但未查证"),
    (r"配置.*在\s*\S+", "描述配置路径但未确认"),
    (r"默认.*是", "描述默认值但未查证"),
    (r"版本.*是\s*\S+", "描述版本但未查证"),
...
def cmd_admit(original: str, corrected: str) -> str:
    """主动认错：生成标准化认错模板并记录"""
    log = load_log()
    log["self_corrections"] += 1
    log["checks"].append({
...
"""
    ...
    try:
        if args.cmd == "check":
            ...
            print(cmd_admit(args.original, args.corrected))
        else:
            parser.print_help()
